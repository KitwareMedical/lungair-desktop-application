...
class Eicu:
    def __init__(self, eICU_dir: str, schema_dir: str):
        ...
        self.respiratory_care_df = pd.read_csv(os.path.join(eICU_dir, "respiratoryCare.csv.gz"), dtype=dtype_dict)
    ...
    def get_fio2_df(self):
        ...
        if self.fio2_df is None:
            ...
            self.fio2_df = fio2_df.assign(
                respchartvalue_float=fio2_df["respchartvalue"].apply(lambda x: x.strip("%")).astype("float32")
            )
        return self.fio2_df
    ...
    def process_fio2_data_for_unitstay(self, unitstay_id: str):
        ...
        fio2_data_with_deltas = fio2_data_with_deltas.assign(
            val_shifted=fio2_data_with_deltas["respchartvalue_float"].shift(1)
        )
        total_fio2_time = fio2_data_with_deltas["delta_t"].sum()
        if (
            total_fio2_time <= 0.0
        ):  # It should be possible to have total_fio2_time be 0, if there is just one fio2 entry (so there are no delta_t's)
            if (
                len(fio2_for_unitstay) > 1
            ):  # If that is not what happened, we need to fix this code because that's a case I haven't thought about
                raise Exception(
                    f"Got total time FiO2 time of 0 when trying to integrate, but there is more than one FiO2 entry. Unit stay ID: {unitstay_id}."
                )
            ...
            average_fio2 = (
                fio2_data_with_deltas["val_shifted"] * fio2_data_with_deltas["delta_t"]
            ).sum() / total_fio2_time

        # Compute total time spent within each FiO2 value bin
        bins = [[start, start + 10] for start in range(0, 100, 10)]
        total_times = [
            fio2_data_with_deltas["delta_t"][
                (fio2_data_with_deltas["val_shifted"] >= start) & (fio2_data_with_deltas["val_shifted"] < end)
            ].sum()
